...
from utils import time_this, _init_weight
"""
...
class Actor(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.mu = nn.Linear(hidden_size, num_outputs).to(device)
...
class Critic(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.linear2 = nn.Linear(hidden_size+num_outputs, hidden_size).to(device)
        self.ln2 = nn.LayerNorm(hidden_size).to(device)

        self.V = nn.Linear(hidden_size, 1).to(device)
...
class DDPG(object):
    ...
    def update_parameters(self, batch):
        ...
        mask_batch = Variable(torch.stack(batch.mask))
        next_state_batch = Variable(torch.stack(batch.next_state))
